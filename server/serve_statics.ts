...
const dir_path = abs_path(import.meta.url, '../client/public')
console.log('serving static files from ', dir_path)

/** 检查路径是否正确（以防在不同目录启动项目时，所导致的相对路径异常） */
export
const check_static_path = () => {
  const static_secret = 'yyyyyyyyyyyyyyyes'
  const secret = Deno.readTextFileSync(dir_path + '/' + static_secret)
...
const serve_statics = async (pathname: string): Promise<Response> => {
  ...
  try {
    let path = dir_path + pathname // !!!! 不能用 join
    ...
    if (err instanceof Deno.errors.NotFound) {
      if (pathname.includes('.')) { // 判定为非页面请求
        ...
        const file = await Deno.open(dir_path + '/index.html') // !!!!! 不能用 join
        return new Response(file.readable)
      }
    } else {
